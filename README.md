...
Зомби-процессы освобождают ресурсы, но блокируют запись в таблице процессов.
...
`set -e` так же прекратит выполнение при возникновении ошибки в первой команде. Таким образом, нет смысла использовать в bash `&&`, если применить `set -e`. 
...
```TEXT
  ...
  -o pipefail - вернет статус последней команды с ошибкой
```
