...
Минусы: Ограничение хранилища (все данные должны поместиться в оперативной памяти). Redis предлагает только базовую безопасность
(нет механизма ролей). Нет языка запросов (как SQL) - теряется гибкость. 
 ```